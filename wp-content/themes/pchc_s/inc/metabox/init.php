...
class cmb_Meta_Box {
	...
	function show() {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( 'multicheck' == $field['type'] || 'posts_multicheck' == $field['type'] ) $field['multiple'] = true;
                ...
                case 'posts_multicheck':
                    ...
                    if( $posts_query->have_posts() ) {
                        while( $posts_query->have_posts() ) : $posts_query->the_post();
                            echo '<li><label><input type="checkbox" name="', $field['id'], '[]" id="', $field['id'], '" value="', get_the_id(), '"', in_array( get_the_id(), $meta ) ? ' checked="checked"' : '', ' />';
	...
	function save( $post_id)  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ! isset( $field['multiple'] ) )
				$field['multiple'] = ( 'multicheck' == $field['type'] || 'posts_multicheck' == $field['type'] ) ? true : false;
